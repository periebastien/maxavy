...
- **Forme** : carré / cercle — **même nombre de points dans les deux cas** (décision produit, screenshots de référence 2026-07-02). En mode cercle, depuis **G14 (2026-07-12)** : **grille polaire** — un point central + R=(N−1)/2 **anneaux concentriques** à distance exacte k×espacement, **8k points par anneau** répartis uniformément en angle (premier point plein nord) → total 1+Σ8k = N², espacement d'arc constant ≈0,79×espacement. Remplace le disque de maille carrée (les « N² points les plus proches ») utilisé de G6 à G13. **Dimension** (dans le plafond du plan). **Espacement** (m).
...
> Le **nombre de points** (= N²) pilote le coût (plafonné par plan) ; l'**espacement** ne change que la couverture (libre). En mode cercle (grille polaire G14), `buildGrid` encode `row`=n° d'anneau et `col`=index dans l'anneau (plus des coordonnées cartésiennes). Le contour et la couverture (diamètre) sont calculés côté front d'après le point le plus éloigné.
>
> **Continuité après changement de grille (G14)** : chaque scan fige sa géométrie (`grid_size`/`grid_spacing_m`/`center`/`shape` — migration 59). Quand l'historique d'un mot-clé mélange plusieurs géométries, `GET /trend` et `GET /competitors/trend` ajoutent des métriques `*_comparable` calculées sur la **zone commune** (intersection exacte des coordonnées lat/lng des points, ≥ 9 points requis — une grille plus petite de même centre/espacement est un sous-ensemble exact de la grande, en carré comme en polaire). Le front trace les courbes sur ces valeurs et pose un **marqueur de rupture** (ReferenceLine) à la date du changement ; la flèche d'évolution est neutralisée entre deux rapports de géométries différentes ; le wizard demande confirmation avant d'enregistrer un changement de grille quand des rapports existent.
