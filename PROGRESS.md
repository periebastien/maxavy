...
| 30 | Gestion équipe | ✅ Terminé (2026-07-06) | Nouveau module `modules/team/` (routes/controller/service) : `GET /team` (owner + membres + invitations en attente, `my_role`), `POST /team/invite` (email + rôle ; email Brevo `sendTeamInviteEmail` ; utilisateur existant → `TeamMember` pending direct, sinon `team_invitations`), `POST /team/accept` (public, token → crée le compte + hash bcrypt si besoin ou rattache le compte connecté, renvoie un JWT), `PUT /team/:id/role` + `DELETE /team/:id` (réservés owner/admin). Table `team_members` **existait déjà** (migration 8, ENUM `admin`/`editor`/`viewer`) → aucune migration destructive. **Nouvelle migration additive 56** `team_invitations` (invités **sans compte** : `team_members.user_id` est NOT NULL donc impossible d'y tracer un inconnu — email chiffré AES-256-GCM + `email_hash` SHA-256 pour lookup + `token_hash` SHA-256, jamais le token en clair, expiration 7 j). Token = `crypto.randomBytes(32)` (pas un JWT), source de vérité en base. **Enforcement des rôles** : `assertAccess(business, userId, { write })` étendu de façon **rétrocompatible** (3ᵉ arg optionnel) — owner toujours tous droits (aucun TeamMember pour lui → non-régression stricte mono-utilisateur), membre non accepté rejeté, `viewer` bloqué en écriture. `{ write: true }` appliqué aux points d'écriture de **customers** (create/update/remove/import), **campaigns** (create/pause/resume/cancel), **invitations** (send), **widgets** (create/update/remove), **reviews** (setReviewTags). Settings déjà `owner_id`-only (plus strict, inchangé). **Frontend** : section « Équipe » dans `SettingsPage` (`components/settings/TeamSection.jsx` — liste, badges statut/rôle, `<select>` de rôle inline, invitation email+rôle, retrait ; actions masquées si `my_role` ≠ owner/admin) ; page publique `/invitation` (`AcceptInvitationPage.jsx` — preview du nom d'entreprise/rôle/email, formulaire création de compte si nouvel invité, sinon connexion, stockage token + redirect `/dashboard`). Route montée dans `app.js`. **Testé en réel** (script API + vraie base, 28/28 PASS) : cycle invite→preview→accept→list→viewer 403 en écriture / 200 en lecture→changement de rôle→editor 201→invitation utilisateur existant (pending)→non-admin 403→retrait→ex-membre 403. Non-régression confirmée (login réel + `/auth/me` + route customers owner OK). Vérifié en preview navigateur (section Équipe rendue, page `/invitation` avec vrai token + état token invalide). Données de test nettoyées (0 résidu). |
| 31 | Profil & sécurité | ✅ Terminé (2026-07-10) | Cahier des charges dédié rédigé et validé avant dev : `MON_COMPTE_DESIGN_FR.md` (périmètre v1/v2, décisions §8). **Backend** (extension du module `auth`, aucun nouveau module ni migration — tous les champs existaient) : `PATCH /auth/me` (whitelist stricte firstname/lastname/phone, tout autre champ ignoré), `PUT /auth/me/password` (401 si mdp actuel faux, 400 si `password_hash` null = compte Google, min 8 car., ≠ actuel — vérifié APRÈS le mdp actuel pour ne rien divulguer, rate limit 20/15 min, email de notification `sendPasswordChangedEmail` best-effort try/catch) ; `GET /businesses` expose `my_role` (owner ou rôle `team_members`). Testé service direct contre la vraie base : 21/21 PASS, données nettoyées. **Frontend** : page `/account` (`pages/AccountPage/`, sections empilées façon Settings) — Profil (avatar Gravatar/Google affiché + note, email lecture seule, prénom/nom/téléphone, save → `setUser` : le nom de la sidebar se met à jour sans re-login), Sécurité (formulaire mdp masqué si `auth_provider==='google'` → encart « Connexion gérée par Google »), Mes entreprises (badges de rôle FR, lecture seule) ; `refreshUser()`/`setUser` exposés par `AuthContext` ; entrée « Mon compte » de `AccountMenu` activée (badge « bientôt » retiré). **Fix découvert en vérif E2E** : le 401 « mot de passe actuel incorrect » déclenchait le handler 401 centralisé (audit sécurité) → déconnexion brutale ; route `/auth/me/password` ajoutée à `AUTH_PATHS` dans `lib/api.js` (le 401 s'affiche désormais inline, vérifié). Vérifié en preview réel : login, page, update profil persisté, erreur mdp inline, navigation menu → `/account`. v2 notées au cahier : changement d'email, « définir un mot de passe » (comptes Google), suppression de compte (RGPD), 2FA, upload avatar. ⚠️ Session menée en parallèle d'une autre session (migration 57 move-plan-to-users) — collision évitée, migration 57 non touchée. Implémentation : 2 sous-agents Sonnet 5 (backend/frontend) + vérif E2E. |
| 32 | Super Admin | ✅ Terminé (2026-07-04, orchestration 3 agents) | Panel `/admin/*` (`AdminLayout` à onglets Plans/Comptes/Modules, réservé `role===superadmin`, absorbe G12). **Plans** : CRUD complet sur `plans-admin/` (name/description/price/monthly_credits/features JSONB/stripe_price_id(_yearly)/active/sort_order, `POST`+`PUT /api/v1/admin/plans[/:id]`, pas de DELETE — `active=false` suffit). **Comptes** : nouveau module `admin-accounts/` — vue cross-tenant inédite (`GET /api/v1/admin/accounts` avec recherche nom/email, changement de plan `PUT .../plan`), ajout de crédits réutilise l'endpoint `POST /credits/add` existant. **Modules hors plan** : nouveau modèle `BusinessModule` (mappe la table `business_modules` jusqu'ici jamais utilisée) + module `admin-modules/` (catalogue statique rank_tracking/reviews/widgets, upsert `enabled`/`activated_at`/`settings` par entreprise) — ⚠️ écriture seule pour l'instant, aucun module métier ne lit encore ce flag (le gating réel passe par `plans.module_quotas`), même posture que le hook `notify_failure` de G10.5. Aucune migration nécessaire (tous les champs existaient déjà). Testé en réel contre la vraie base sur les 3 volets (créations/mises à jour/rejets de validation, données de test nettoyées). Reste : vérification visuelle en preview navigateur (frontend piloté manuellement par l'utilisateur cette session). **Complément (2026-07-04, retour utilisateur)** : les plafonds « nombre d'entreprises » et « nombre de localisations » par plan manquaient — ajoutés. Migration 55 (`plans.max_businesses`/`max_locations`, NULL=illimité, additive). Enforcement réel (pas juste un champ admin) : `business.service.js create()` bloque la création si le propriétaire atteint le plafond (le plus permissif entre le plan Gratuit et ceux de ses entreprises existantes — un plan illimité rend l'ensemble illimité) ; `location.service.js create()` bloque si l'entreprise atteint son plafond de localisations (plan de l'entreprise, ou Gratuit si `plan_id` NULL). Champs exposés dans `GeneralForm` (`AdminPlansPage.jsx`). Complète une promesse du cahier des charges jamais implémentée (« toujours borné par le nombre de localisations autorisé par le plan », répété §9.5/§10). Testé en réel (4 scénarios : blocage à la limite, déblocage en l'augmentant, sur Atlasimmobilier + un user/business de test jetables) — tous les plans restaurés à `null` après test, zéro régression (tout illimité tant que le Super Admin ne configure rien). **Bugs découverts et corrigés en vérifiant l'accès Super Admin (2026-07-04-2026-07-05)** : (1) `periebastien@gmail.com` avait le rôle `owner` en base — le cahier/CLAUDE.md le documentait comme Super Admin mais rien ne l'avait jamais réellement posé (`UPDATE users SET role='superadmin'`, corrigé) ; ⚠️ piège découvert au passage : le rôle est lu depuis le **JWT** (généré à la connexion), pas relu en base à chaque requête — changer le rôle en base ne prend effet qu'après une **reconnexion** (nouveau token). (2) `LoginPage.jsx`/`GoogleButton.jsx` redirigeaient toujours vers `/dashboard` après connexion, quel que soit le rôle → un compte superadmin sans entreprise tombait en boucle sur `/onboarding` (`RequireBusiness`) ; corrigé pour rediriger vers `/admin/plans` si `role==='superadmin'`. (3) `OnboardingPage.jsx` n'avait **aucun** moyen de se déconnecter pour un utilisateur sans entreprise (le lien « Annuler » n'apparaissait que si `hasBusinesses`) — utilisateur bloqué dessus ; ajout d'un lien « Se déconnecter » toujours visible. **4ᵉ onglet « Planning » (2026-07-08)** : nouveau module `admin-schedule/` (`GET /api/v1/admin/schedule/geogrid-month`) — vue cross-tenant en lecture seule de tous les rapports geogrid (positionnement/hitmap) prévus du jour jusqu'à la fin du mois, toutes entreprises confondues (utilise `computeNextRunAt` existant, garde-fou 60 occurrences/config). Page `AdminSchedulePage.jsx` + onglet dans `AdminLayout` ; un bouton « Se déconnecter » a aussi été ajouté au layout admin (absent jusqu'ici). **Fix associé** `BusinessContext.jsx` : `isLoading` reposait sur un booléen qui ne se réinitialisait pas correctement entre deux connexions (`businesses` state jamais remis à « en cours de chargement ») — remplacé par une sentinelle `null` (non chargé) vs `[]` (chargé, vide), corrige une source probable de faux `hasBusinesses=false` juste après une reconnexion. Vérifié en réel : endpoint testé contre la vraie base (7 rapports à venir sur Atlasimmobilier, dates/fréquences correctes), syntaxe validée sur tous les fichiers. |
| 32b | Facturation par propriétaire & coûts crédits | ✅ Terminé (2026-07-11) | **Refonte du rattachement plan/crédits** : le plan d'abonnement et le pool de crédits vivent désormais sur le **user propriétaire** (`users.plan_id`, `users.credit_balance`, `stripe_customer_id`/`stripe_subscription_id`) et non plus par entreprise — un plan multi-entreprises couvre toutes les entreprises de l'owner. Migration additive 57 (backfill : plan le plus cher de l'owner, somme des soldes ; colonnes `businesses.plan_id`/`credit_balance` laissées mortes, plus lues nulle part). Helper central `services/plan-resolver.js` (`getPlanForBusiness`/`getPlanForOwnerId`, fallback Gratuit) consommé par reviews/rank-tracking/locations/businesses/widgets. Débits atomiques sur l'owner (`User.increment`/`decrement`) dans credits/invitations/cron ; table `credits` garde `business_id` pour l'audit. Stripe : metadata `user_id`, webhooks écrivent sur `users`. Admin : `GET /admin/accounts` groupé par owner (+ noms des localisations par entreprise), `PUT /admin/accounts/owner/:userId/plan`. Front : page Comptes en cartes par propriétaire, `PricingPage` lit `user.plan_id`, Dashboard + Sidebar affichent le pool via `/credits/balance` (fini les soldes par entreprise). **Packs & coûts configurables** : migration 58 — tables `credit_packs` (seed des 3 packs ex-en-dur ; checkout Stripe lit la base) et `credit_costs` (email=1, SMS=5, WhatsApp=5, geogrid_point=2). Module `admin-credits/` (CRUD packs avec `price_per_credit`, upsert coûts, cache 60s `services/credit-costs.js`) + onglet Super Admin « Crédits » ; prix par crédit affiché aussi sur la page client. Débit email paramétrable ; **débit geogrid activé** : coût × points au lancement du scan (`submitScanForKeyword`), une seule fois (les retries A/B/C rappellent `postScanTasks` sans re-débiter), solde insuffisant → 402 en manuel, scan `failed` terminal sans retry en cron. Vérifié en réel (API + base). Implémentation : sous-agents Sonnet, sessions parallèles (Mon compte, rebranding) — commits sélectifs. |
...
| 35 | Config serveur | ✅ Terminé (2026-07-11) | **Plesk + Phusion Passenger** (et NON PM2/Nginx comme prévu initialement). Serveur OVH `ns3181892`, domaine `gmbmanager.ai`. Application Root `/httpdocs/backend`, Startup File `src/app.js`, Application Mode `production`, Document Root `/httpdocs/backend/public` (**contrainte Passenger** : le doc root doit être un sous-dossier de l'app root). Le **backend sert le SPA React** depuis `backend/public` + fallback `app.get('*')` pour React Router ; CSP helmet désactivée (Google Maps/OAuth chargés depuis des domaines tiers). Déploiement continu via **Plesk Git** (dépôt `periebastien/maxavy`, branche `main`, chemin de déploiement `/httpdocs`) ; le front buildé est committé dans `backend/public` (script `npm run deploy:build` dans `frontend/` : build vite + copie auto). Après pull → **Restart App** (+ NPM install si nouvelle dépendance). |
| 36 | Déploiement (mise en ligne) | ✅ Terminé (2026-07-11) | **En ligne sur https://gmbmanager.ai.** ⚠️ Correction majeure de doc : la BDD n'est **PAS** sur OVH distant — c'est un **PostgreSQL local au serveur** (en dev c'était le `localhost` du PC). Base `postgreGmb_`, user `gmbpostgre_user42` ; schéma + données migrés via **dump `pg_dump`** (restauré par `psql` lancé depuis une **tâche planifiée Plesk** — pas de phpPgAdmin, terminal web sans copier-coller). `.env` de prod à **`/httpdocs/.env`** (chargé par app.js via `../../.env`, au-dessus du doc root donc non exposé) + overrides dans « Variables d'environnement personnalisées » de Plesk (NODE_ENV=production, URLs Google prod, APP_URL, STORAGE_PATH — dotenv n'écrase pas les vars déjà définies, les overrides Plesk priment). **SSL DB piloté par `DB_SSL`** (commit `8ca6e13`) : off pour une base locale ; avant il était forcé quand NODE_ENV=production, ce qui cassait la connexion à une base locale sans SSL. Garde-fou anti-double-cron : verrou consultatif `pg_try_advisory_lock` dans app.js (une seule instance lance les crons). **Restant prod** : basculer les URIs OAuth Google + le webhook Stripe sur `https://gmbmanager.ai` ; garder l'app en **1 instance** Passenger. (Compte `cogitowebnet@gmail.com` conservé volontairement — compte de travail de l'utilisateur, ne pas reverter. Dumps locaux supprimés.) |

### Correctifs post-mise-en-ligne (2026-07-11)
Bugs découverts en conditions réelles juste après le déploiement, corrigés et poussés :
- **`8ca6e13`** — SSL DB piloté par `DB_SSL` (et non `NODE_ENV`) : sinon la connexion à une base PostgreSQL locale (sans SSL) échoue au boot en prod (`app.js` faisait `process.exit(1)`).
- **`4a74f1a`** — `widgets.embed_code` recalculé **à la lecture** (list/getOne/create/update) depuis l'`APP_URL` courante, au lieu de la valeur figée en base. Réparait les widgets créés en dev puis importés en prod via dump : leur snippet pointait sur `http://localhost:5173`.
- **`09fe59f`** — **CORP** : les 3 routes publiques widgets (`/:id/embed.js`, `/runtime.js`, `/:id/public`) renvoient `Cross-Origin-Resource-Policy: cross-origin`. helmet mettait `same-origin` par défaut → le navigateur bloquait le chargement des scripts sur les sites clients (`ERR_BLOCKED_BY_RESPONSE.NotSameOrigin`), widget invisible. Reproduit en cross-origin réel puis corrigé.
...
| G14 | Continuité après changement de grille + cercle polaire | ✅ Terminé (2026-07-12) | Grille cercle refondue en **grille polaire** (anneaux 8k points, total N² conservé) ; métriques comparables « zone commune » dans `GET /trend` + `/competitors/trend` (migration 59 `geogrid_scans.shape`) ; marqueurs de rupture sur les courbes ; flèche d'évolution protégée ; avertissement wizard avant modification de grille. Voir détail ci-dessous |
...
### Détail session G14 — Continuité après changement de grille + cercle polaire (2026-07-12)

Problème posé par l'utilisateur : que deviennent les rapports quand on change la carte de positionnement (ex. 9×9 → 7×7) ? Constat : chaque scan fige sa grille (aucune corruption), mais les métriques agrégées (ATRP/ARP/SoLV) portent sur des populations de points différentes → saut artificiel dans les courbes, flèche d'évolution mensongère, aucun signal de rupture. Orchestration 3 agents Sonnet (backend polaire / backend S1-S2 / frontend S1-S2-S3-S5).

**Cercle polaire (`geogrid.utils.js`)** — la forme `'circle'` (ex-disque de maille carrée) devient une **vraie grille polaire** : centre + R=(N−1)/2 anneaux à distance exacte k×spacing, **8k points par anneau** (premier point plein nord, sens horaire) → total 1+Σ8k = **N² conservé** (même coût crédits que le carré), espacement d'arc constant ≈0,79×spacing (densité homogène). Encodage dans les colonnes existantes : `row`=anneau, `col`=index (plus des coordonnées cartésiennes pour cette forme). Le carré est strictement inchangé. Option A actée : remplacement direct, pas de nouvelle valeur de shape ; les anciens scans cercle restent affichables (points figés en base).

**S1 — métriques « zone commune » (backend)** — migration additive **59** (`geogrid_scans.shape`, snapshot à la création du scan) ; helper partagé `trend-compat.js` : signature géométrique par scan (`taille|espacement|centre|forme`), et si ≥2 signatures dans l'historique d'un mot-clé → intersection **géographique exacte** des points (clés lat:lng arrondies 1e-7 — une grille plus petite de même centre/espacement est un sous-ensemble exact de la grande ; en polaire, R anneaux = préfixe exact de R+1). Si ≥9 points communs : `GET /trend` ajoute `atrp/arp/solv_comparable` + `points_comparable` par scan, `GET /competitors/trend` ajoute `avg_position_comparable` par point de série (JSONB `competitors` chargé uniquement sur ce chemin). Cas nominal (grille jamais changée) : zéro coût ajouté.

**S2/S3/S5 (frontend)** — `lib/geogrid-geometry.js` (`isGeometryBreak`, `detectGeometryBreaks`, tolérance shape null→valeur pour les scans d'avant la colonne) ; courbes Suivi/Concurrents tracées sur `*_comparable ?? brut` ; **marqueurs de rupture** = `ReferenceLine` pointillée + label (« Grille 9×9 → 7×7 », « Espacement 500 m → 750 m », « Centre déplacé », « Forme modifiée ») via prop `markers` de `TrendChart` (piège Recharts 3 contourné : label passé en enfant `<Label>` plutôt qu'en prop objet) ; **flèche d'évolution** remplacée par « — » + tooltip quand les deux rapports comparés n'ont pas la même géométrie ; **wizard Config** : bandeau ambre + double-clic de confirmation avant d'enregistrer un changement de grille quand des rapports existent (message « zone commune » si seule la taille change, « rupture de comparabilité » si espacement/centre/forme).

**Vérifié en réel** (compte de test, données Atlasimmobilier qui contiennent justement un changement d'espacement 500→1000 m le 02/07) : buildGrid polaire (49 pts, anneaux à 500/1000/1500 m exactement, carré intact), migration appliquée, `GET /trend` renvoie les champs comparables calculés sur la zone commune (9 pts), marqueur « Espacement 500 m → 1000 m » rendu dans le SVG de la courbe, `grid-preview` cercle OK, messages S5 affichés sans PUT parasite. Incidents de vérif sans rapport avec le code : viewport headless à 0 px (ResponsiveContainer vide) + rate-limit 429 après rechargements répétés (résolu par redémarrage backend — piège déjà connu).
...
**Correctif transverse isolation par localisation (2026-07-04)** : toutes les pages sidebar suivent désormais `activeLocation` (ReviewsPage recâblée — son dropdown local supprimé, migration 53 `customers.location_id` NULL=toutes fiches, invitations persistent enfin `location_id`, campagnes/widgets/stats filtrés, Dashboard par fiche, faille widgets cross-tenant fermée via `assertOwnership`). **Audit corrigé (2026-07-04)** : `POST /credits/add` protégé par le nouveau `middlewares/super-admin.middleware.js`, index unique reviews scopé `(location_id, platform, external_id)` (migration 54, upsert aligné), `business.middleware.js` supprimé. Détail : `PROGRESS.md` « Correctif transverse ».
**Sessions 29 et G12 terminées (2026-07-04, orchestration multi-agents)** : 29 — Paramètres entreprise (migration 54 `add-settings-fields-to-businesses` : logo_url/contact_email/contact_phone/address/notification_prefs JSONB ; `PATCH /businesses/:id` étendu avec validation slug kebab-case + unicité ; SettingsPage enrichie + entrée Sidebar ; testé 10/10 contre la vraie base). G12 — module `plans-admin` (`GET /api/v1/admin/plans`, `PUT /:planId/rank-tracking` avec validation stricte, page minimale `/admin/plans` gated `role==='superadmin'`, pas de cache → effet immédiat). ⚠️ Collision bénigne : deux migrations portent le timestamp 20260704000054 (tables différentes, toutes deux appliquées). **Session 32 — Super Admin terminée (2026-07-04, orchestration 3 agents)** : panel `/admin/*` (onglets Plans/Comptes/Modules, `AdminLayout` partagé), absorbe et étend G12. CRUD complet des plans (prix, crédits, features, stripe ids, actif). Vue cross-tenant inédite des comptes (`admin-accounts/`) avec changement de plan + ajout de crédits (réutilise `POST /credits/add`). Activation de modules hors plan (`admin-modules/` + modèle `BusinessModule` — la table existait depuis longtemps mais n'avait jamais été utilisée) : **écriture seule pour l'instant**, aucun module métier ne lit ce flag (gating réel = `plans.module_quotas`), à consommer plus tard comme `notify_failure` (G10.5). Bug corrigé au passage : `periebastien@gmail.com` n'avait jamais eu le rôle `superadmin` réellement posé en base (juste documenté) + `LoginPage`/`GoogleButton` redirigeaient toujours vers `/dashboard` (→ onboarding en boucle pour un compte sans entreprise) ; espacement de grille (G12) contraint désormais aux mêmes valeurs que le wizard (250/500/750/1000/1500/2000 m, n'affecte pas le nombre de points). **Complément (2026-07-04)** : plafonds « max entreprises »/« max localisations » par plan ajoutés (migration 55, NULL=illimité) avec **enforcement réel** à la création (business.service.js/location.service.js), pas juste un champ admin — complète une promesse du cahier jamais tenue. Prochaine session : **30 — Gestion équipe** ou **G11 — Rapport email (v1)**. Bug corrigé (2026-07-01) : modèle `Review` avait `updated_at` absent de la table → cassait toute lecture d'avis, fix `updatedAt: false`. Config Stripe (clés + stripe_price_id des plans) à faire séparément.
Voir `PROGRESS.md` pour le détail complet.


**Correctif avis — statut de synchro + backfill dynamique (2026-07-22, commit `13391c8`)** : `GET /reviews/sync/status` renvoie `last_job` (statut/erreur/avis importés, filtré `competitor_place_id IS NULL` — les jobs concurrents ne comptent plus dans `running`) ; page Avis : bandeaux « Synchronisation en cours » / échec / « X avis importés » ; `triggerSync` remonte la vraie erreur si aucune location enqueue ; **backfill à profondeur dynamique** : si `total_reviews_count > depth` du backfill, relance automatique unique à `depth = total arrondi à la dizaine sup.` (plafond `REVIEWS_BACKFILL_MAX_DEPTH`, défaut 2000, garde-fous anti-boucle). Déployé en prod ; Altitude Révision (258 avis > plafond 200) : re-backfill forcé (`reviews_backfilled_at=NULL`) → 258/258 avis en base, vérifié. Diagnostic associé : la connexion OAuth Google n'est jamais confrontée au `place_id` (proposition B « vérif compte↔fiche » restante).

**Vérification compte Google ↔ fiche (2026-07-22, commit `e24603a`)** : au callback OAuth, `verifyLocationMatch` liste les comptes (My Business Account Management API) et leurs fiches (Business Information API, `metadata.placeId`) et compare aux `google_place_id` du business. Migration 60 (`google_connections.verified_location_match` BOOLEAN nullable + `verification_error` TEXT). Dégradation douce : erreur/quota/403 → `null`, le callback n'échoue jamais ; `GET /google/status` expose le résultat ; Settings affiche badge vert « Fiche vérifiée » / avertissement orange « le compte ne semble pas gérer la fiche ». ⚠️ Les API Business Profile nécessitent une approbation Google (quota 0 par défaut) → tant qu'elle n'est pas obtenue sur le projet Cloud, la vérif restera `null` (cause en base dans `verification_error`). Déployé + migré en prod.

**Widgets — badge compact et lisible (2026-07-25, commits `b7b6831` / `460222e` / `2ecfd02`)** : trois réglages revus dans le builder badge. (1) `containerPadding` renommé « **Marge extérieure** » côté badge (espace autour, `.lcg-wrap`) — reste « Marge intérieure » côté carrousel où il s'agit bien du padding de `.lcg-car` ; deux entrées `FIELDS` sur la même clé, filtrées par `styles`. (2) Nouveaux `badge.paddingY`/`paddingX` (padding interne réel de `.lcg-bc`/`.lcg-bf`), sentinelle **`-1` = automatique selon la taille** → aucun widget existant ne change de rendu ; la surcharge mobile `6px 12px` ne s'applique plus dès qu'un padding manuel est posé. (3) `badge.googleMark` (enum `text`/`logo`/`both`/`none`) remplace la case commune `showGoogleLabel` : le G multicolore (`googleG()`, déjà utilisé par le carrousel) peut se substituer au mot « Google » — compact 208 → **173 px**, **140 px** sans l'étoile ; en style encadré le G coiffe la carte et le mot suit « 25 avis Google ». Rétrocompat dans `mergeDefaults` (`showGoogleLabel:false` → `googleMark:'none'`), `showGoogleLabel` conservé en base mais retiré du builder. **Réglages rendus contextuels** : « Logo Google » (`showGoogleLogo`) n'est plus affiché pour le badge (il n'y agissait pas), « Mention Google » plus affichée pour le carrousel. Séparateur `.lcg-sep` : trait 1 px + 8 px de marge → glyphe « **|** » + 4 px (3 px en mobile). Vérifié en exécutant le vrai moteur de rendu dans le navigateur (largeurs mesurées sur les 4 valeurs de `googleMark`, styles compact et encadré) ; capture d'écran impossible (panneau Browser non affiché). Déployé en prod, aucune migration.

## Widgets — en-tête carrousel repensé + taille des avatars (2026-08-01)

**Taille des avatars en px** : nouvelles clés `badge.avatarSize` / `carousel.avatarSize` (défaut `30`, bornes 20–64, entrée unique `avatarSize` dans `NUM_BOUNDS`). Le runtime calcule la taille selon `payload.type` ; police de l'initiale = taille×0,43 ; chevauchement des avatars empilés du badge = `-taille/3`. Champ « Taille des avatars (px) » ajouté au builder (section apparence des deux scopes).

**Refonte de l'en-tête du carrousel** — nouvelles clés `carousel.headerPosition`/`headerQualityLabel`/`headerGoogleMark`/`headerTextColor`/`headerMutedColor` :
- `headerPosition` : `left` (**nouveau défaut**, changement de rendu par défaut validé par l'utilisateur) | `top`.
- **`left`** (style Elfsight) : bloc vertical à gauche des cartes — libellé qualité en majuscules (`headerQualityLabel`, `auto` = Excellent/Très bien/Bien selon la moyenne, texte libre possible, vide = masqué), grandes étoiles, « Basée sur **N avis** », logotype Google complet multicolore (nouveau SVG `googleWordmark()`). Bloc 190px centré verticalement (flex row `.lcg-car-hl`/`.lcg-car-body`), repasse en colonne (en-tête au-dessus) sous 640px. Compatible slider/grid/list.
- **`top`** : rendu aligné sur le badge compact — [G logo]|[moyenne]|[étoiles] puis séparateur vertical « | » (`.lcg-sep` réutilisée) + « N avis Google » ; `headerGoogleMark` (`text`/`logo`/`both`/`none`, défaut `text`) pilote logo/mot Google.
- `headerTextColor`/`headerMutedColor` (défaut `auto` → retombent sur texte/texte secondaire) — corrige un manque signalé : l'en-tête n'était pas réglable indépendamment des cartes (texte clair sur cartes sombres = en-tête invisible sur fond de page clair).
- Builder : champs contextuels via nouveau mécanisme `when: [{key, equals}]` dans `frontend/src/lib/widget-schema.js` (export `fieldVisible`, appliqué dans `WidgetBuilderPage.jsx`) — « Position de l'en-tête », « Libellé (auto / texte) » (left seulement), « Mention Google (en-tête) » (top seulement), 2 couleurs d'en-tête ; tous visibles seulement si « En-tête récap » (`showHeader`) actif.

Rétrocompatible côté données (`mergeDefaults` complète les configs existantes), **aucune migration** ; le rendu par défaut des carrousels existants passe à l'en-tête gauche (voulu, pas une régression).

Fichiers modifiés : `backend/src/modules/widgets/widget.defaults.js`, `backend/src/modules/widgets/widget.runtime.js`, `frontend/src/lib/widget-schema.js`, `frontend/src/pages/WidgetBuilderPage.jsx`.

Vérifié en Playwright headless : positions left/top, logo, couleurs custom, avatars 30→52px, badge 48px, responsive mobile, style grid, builder (aperçu live + champs conditionnels).

## Préparation examen Google Business Profile API — site public (2026-08-03)

Objectif : cohérence du site public avec la demande d'accès aux API Google Business Profile (scope `business.manage`) en cours d'examen chez Google.

**Pages légales en HTML statique pur** (`backend/src/static-pages/` : `confidentialite.html`, `cgu.html`, `mentions-legales.html`), servies par Express dans `app.js` **avant** le fallback SPA, aux URLs inchangées `/confidentialite`, `/cgu`, `/mentions-legales` (déclarées dans la console Google Cloud — ne jamais les changer). Motif : lisibles sans JavaScript par les crawlers/reviewers Google (vérifié en Playwright JS désactivé). Hors de `backend/public` (écrasé par `deploy:build`). Les pages React équivalentes sont **supprimées** (source unique) ; liens footer en `<a>` (chargement complet) ; proxy Vite ajouté pour ces 3 chemins en dev.

**Contenus** : `/confidentialite` — section « Données Google » complète (OAuth 2.0, scope complet, accès limité aux fiches dont l'utilisateur est propriétaire/gestionnaire, finalités, phrase Limited Use verbatim exigée par Google, révocation myaccount + déconnexion, conservation 12 mois max, OVH France). `/cgu` — §3 scindé en « 3.1 via les API Google Business Profile » (édition fiches, consultation/réponse avis, stats/mots-clés, multi-établissements) / « 3.2 indépendant des API Google » (collecte d'avis email/SMS/QR, widgets, geogrid + veille via DataForSEO) avec **engagement anti-review-gating** (invitations à tous les clients, formulation neutre, sans contrepartie ni collaborateur nommé) ; §5 renforcé dans le même sens. Home refondue en deux blocs badgés « Via les API Google Business Profile » / « Indépendant des API Google » + encart « accès réservé aux clients de l'agence Makemerank » (répété sur les pages légales). Dates au 3 août 2026.

**Inscription libre fermée** (cohérence « pas de self-service ») : `POST /auth/register` → 403 avec message explicite ; **Google Sign-In ne crée plus de compte** pour un utilisateur inconnu (403, mais lie toujours google_id à un compte email existant) ; route `/register` + `RegisterPage.jsx` supprimées, lien « S'inscrire » de `/login` remplacé par « accès réservé — nous contacter ». Les invitations d'équipe (`POST /team/accept`, création de compte propre dans `team.service.js`) ne passent pas par register → non impactées, vérifié. ⚠️ Conséquence assumée : **plus aucun chemin de création de compte client** hors invitation d'équipe — prévoir une création de compte par le Super Admin (panel `/admin/*`) pour l'onboarding des nouveaux clients de l'agence.

Écarts restants signalés (non bloquants pour le site, à traiter avant/pendant l'examen) : les fonctionnalités API (édition fiches, réponse avis, stats) ne sont pas encore implémentées — seul usage réel actuel : `verifyLocationMatch` au callback OAuth ; Google demande souvent une démo vidéo du scope en usage. SMS : Twilio toujours en stub, mention conservée au catalogue.

Vérifié : 3 pages légales HTTP 200 en HTML brut (contenu complet sans JS), register 403, login 401 inchangé, home/login build prod OK (Playwright headless + curl). Déploiement prod : `git pull` + Restart App dans Plesk (le build est committé dans `backend/public`).

**2ᵉ passe — honnêteté livré vs prévu (2026-08-03, commit `5063f6c`)** : audit du code préalable → seul le socle OAuth est livré côté API Google (connexion/déconnexion, jetons chiffrés, `verifyLocationMatch` en lecture accounts/locations) ; édition de fiches, publication de réponses aux avis et Performance API **non implémentées**. Corrections en conséquence, aucune ligne de code métier touchée :
- **Home** : les 3 fonctions non livrées passées au **futur proche** (« bientôt », « arrive prochainement », « seront prochainement réunis ») + mention « en cours de déploiement » ; multi-établissements et consultation d'avis restent au présent (livrés).
- **CGU §3.1** : mentions « *disponible prochainement* » sur les 3 fonctions + clause « disponibilité conditionnée à l'accès accordé par Google » ; **consultation des avis reclassée en §3.2** (source réelle = DataForSEO, pas l'API Google — l'ancienne rédaction était inexacte) ; multi-établissements précisé (vérification que le compte Google connecté gère les fiches).
- **Confidentialité §3** : paragraphe « Finalité » scindé en **« Accès en lecture »** (infos de fiche, stats, mots-clés, avis) / **« Accès en écriture »** (modification de fiches, réponses aux avis) — écriture uniquement **sur action explicite de l'utilisateur**, « aucune action automatique ou non sollicitée » ; clauses Limited Use/chiffrement/révocation intactes.
Vérifié en prod après déploiement SSH (fetch + checkout + restart) : 16/16 contrôles de contenu Playwright headless sur les 3 pages. Rapport de synthèse remis pour l'assistant qui remplit le formulaire Google.
