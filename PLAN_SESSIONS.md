...
| 32b | Facturation par propriétaire & coûts crédits | ✅ Fait (2026-07-11) — plan + pool de crédits portés par le user owner (migration 57, `plan-resolver.js`, Stripe/webhooks sur users, page Comptes groupée par propriétaire) ; packs de crédits gérables en base + coûts des actions configurables (migration 58, module `admin-credits/`, onglet admin « Crédits », débit email paramétrable, débit geogrid 2 crédits/point activé). Voir `PROGRESS.md` Phase 9 |
...
| G8 | Frontend — Configuration (wizard) | ✅ Fait (2026-07-02) — G8.1 (Grille) + G8.2 (Mots-clés/Planning) + G8.3 (Concurrents + `GET /competitors/detected` + récap + `POST /runs` + mode édition). Ancienne `GeogridPage` conservée à `/positionnement/suivi` jusqu'à G9. Voir `PROGRESS.md` Phase 11. |
