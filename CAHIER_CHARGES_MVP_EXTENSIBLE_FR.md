...
➡️ **Cas particulier — module « Suivi de positionnement » (geogrid, §9.5) :** le **classement local** (le rang d'une fiche vu depuis un point GPS précis) n'est **pas** fourni par la Places API (elle renvoie un tri popularité/distance ≠ ranking Maps réel, et sa ToS interdit de stocker/afficher un historique de classement dérivé de son contenu). Ce rang est obtenu via une **API SERP tierce géolocalisée (DataForSEO)** — c'est le fournisseur qui gère proxys/CAPTCHA. La Places API reste utilisée uniquement pour géocoder/afficher les fiches. Détail complet dans `GEOGRID_DESIGN_FR.md`.
...
| Module | Description | Dépendances |
|---|---|---|
| **Suivi de positionnement (geogrid / heatmap)** | Voir détail §9.5 — classement local par grille GPS sur mots-clés cibles, en heatmap + historique | API SERP tierce géolocalisée (DataForSEO). Pas d'OAuth, pas de proxys à gérer côté nous |
...
├─ Positionnement (geogrid)    (voir §9.5)
├─ Publications GBP             (priorité 2 — posts)
└─ Photos GBP                   (priorité 2 — photos géolocalisées)
```

Vu que les modules Publications/Photos partagent la logique de planification et la même finalité (alimenter une fiche GBP en contenu), ils pourront aussi être présentés comme deux onglets d'une même section "Contenu GBP" plutôt que deux liens séparés — décision purement visuelle à trancher au moment du design, sans impact sur l'architecture backend (qui reste modulaire dans tous les cas). Le module « Positionnement » reste une entrée à part (finalité différente : mesure, pas publication).

### 9.5 Détail — Module « Suivi de positionnement Google Maps (geogrid / heatmap) »

> 📐 Spécification technique du socle (grille, provider, cron, scans) : **`GEOGRID_DESIGN_FR.md`**.
> 🧭 **Refonte UX + fonctionnelle** (assistant de configuration en étapes, page Suivi en lecture seule, suivi de **concurrents**, **rapport email** au client, planification fine, courbes) : cahier des charges dédié **`GEOGRID_REFONTE_FR.md`** — sessions G5→G12 (`PLAN_SESSIONS.md` Phase 11). Cette section donne la vue produit d'ensemble.
>
> **État :** socle **G1→G4 livré** (scan réel, heatmap, métriques, cron, gating). La refonte réorganise le module en 3 espaces — **Configuration** (wizard : grille déplaçable + forme carré/cercle + mots-clés + planning + concurrents), **Suivi** (résultats, courbes d'évolution), **Concurrents** (comparaison) — et ajoute la planification précise (mensuel/hebdo/quotidien selon plan, heure + jour + fuseau par localisation), les concurrents (coût data nul) et le rapport email configurable.

**Objectif :** mesurer et suivre dans le temps le **classement local** d'une fiche Google sur des mots-clés cibles, non pas en un point mais **sur une grille géographique** autour de l'établissement, restitué en **heatmap** (comme Local Falcon / BrightLocal). Un rank-checker classique interroge Google depuis un seul point et rate l'essentiel : sur Maps, le rang change tous les quelques centaines de mètres (une fiche peut être #1 à son adresse et #12 à 2 km).

**Fonctionnement anticipé :**
- Le client suit 1 à N **mots-clés** par localisation (ex. « chirurgien esthétique Paris »).
- Pour chaque mot-clé, on génère une **grille N×N** de points GPS centrée sur la fiche (défaut 7×7, espacement 0,5 km). Point central et cible = `locations.lat/lng` + `locations.google_place_id` (déjà en base — rien à ajouter côté ancrage).
- **Pour chaque point**, une recherche locale géolocalisée à ces coordonnées ; on relève le **rang** de la fiche dans les résultats (Top 20 ; absente = « non classée »). On stocke aussi les concurrents présents à ce point.
- Restitution : **heatmap** (pastille verte 1-3 / orange 4-10 / rouge 10+ ou absente), métriques agrégées **ARP** (rang moyen là où on est visible), **ATRP** (rang moyen sur toute la grille), **SoLV** (% de points dans le Top 3), rang par quadrant (NW/NE/SW/SE), et snapshot note + nombre d'avis.
- **Historique / timeline** : rejouer le même scan à intervalle régulier (cron hebdomadaire par défaut) crée des points d'historique → courbes d'évolution ARP/ATRP/SoLV et comparaison dans le temps.

**Source de données — décision : DataForSEO (voir §5).** Le classement local **n'est pas** fourni par la Places API (tri popularité/distance ≠ ranking réel ; ToS restrictive sur le stockage/affichage). Il vient d'une **API SERP tierce géolocalisée**, DataForSEO (paramètre `location_coordinate = lat,lng,zoom`, 1 point = 1 requête, ~0,0006 $/req). Le fournisseur gère proxys/rotation/CAPTCHA → **aucun proxy à gérer côté Locagain**. La difficulté n'est pas l'IP mais la géolocalisation fine de chaque point, portée par le paramètre de coordonnée. Le provider est **abstrait** derrière une interface (bascule SerpApi/Oxylabs possible sans refonte). On stocke **notre métrique** (un rang dans le temps), pas des copies de contenu Google → dérivé analytique conforme, comme tout le marché du geogrid.

**Tables dédiées prévisibles** (additives, isolées par `business_id`/`location_id`, non chiffrées — pas de données perso) :
- `geogrid_keywords` (business_id, location_id, keyword, grid_size, grid_spacing_m, frequency, active)
- `geogrid_scans` (business_id, location_id, keyword_id, center_lat/lng, status, arp, atrp, solv, rating_snapshot, review_count_snapshot, scanned_at) — un scan = un point d'historique
- `geogrid_points` (scan_id, business_id, row, col, quadrant, lat, lng, rank, competitors JSONB)

**Emplacement :** module backend `modules/rank-tracking/` (déjà cité en exemple §7.1) + cron `jobs/scan-geogrid.js` (hebdo, 4h) ; page frontend `/positionnement` (heatmap sur carte Google, périmètre = localisation active). Ajoute 2 dépendances front (carte + chart, aucune présente).

**Facturation — inclus par plan (pas de crédits par scan) :** gating via `plans.module_quotas.rank_tracking` (JSONB, **éditable en Super Admin**). Le plan fixe les **plafonds** (`max_keywords`, `max_grid_size`, `allowed_shapes`, `allowed_frequencies`, `max_competitors`), l'utilisateur choisit **dans ces limites**. **Gratuit : exclu. Starter : 5 mots-clés / 7×7 / mensuel-hebdo / 3 concurrents. Pro : 15 / 9×9 / mensuel-hebdo / 5. Agence : 50 / 13×13 / +quotidien / 10.** Toujours borné par le nombre de localisations du plan. Coût data maîtrisé (~0,76 $/mois/entreprise pour 7×7 × 3 mots-clés hebdo, queue Priority DataForSEO) ; le cercle réduit le nombre de points (~−30 %) et le suivi de concurrents est **sans surcoût data**.

**Point de vigilance :** le coût est **quadratique** en taille de grille (5×5=25 pts vs 9×9=81) → grille et fréquence bornées par plan, throttle des appels, scan « à la demande » (Live, plus cher) réservé à l'action manuelle depuis l'UI.
...
- **Starter** : 5 mots-clés / grille max 7×7 / carré+cercle / mensuel-hebdo / 3 concurrents.
- **Pro** : 15 / 9×9 / carré+cercle / mensuel-hebdo / 5 concurrents.
