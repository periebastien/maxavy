...
- **Restant** : session 30 (gestion équipe — table `TeamMember` seule existe), session 22 2ᵉ temps (répondre aux avis — DataForSEO ne publie pas, canal d'écriture à trancher : API GMB ou saisie manuelle + IA), G11 (rapport email geogrid + consommation du hook `notify_failure`), Stripe prod (`stripe_price_id` des plans vides), pages légales/RGPD, ops (PM2/Nginx/SSL, backups, monitoring, `/health`), tests automatisés/CI.
...
- Prochaine session : **30 — Gestion équipe** ou **G11 — Rapport email (v1)**.
