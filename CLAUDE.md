...
- **Préparation examen Google API (2026-08-03)** : pages légales `/confidentialite`, `/cgu`, `/mentions-legales` désormais en **HTML statique pur** (`backend/src/static-pages/`, routes Express avant le fallback SPA, URLs inchangées — déclarées dans la console Google Cloud, lisibles sans JS) ; pages React équivalentes supprimées (source unique). CGU/home structurées « via les API Google Business Profile » vs « indépendant » + engagement anti-review-gating + encart « accès réservé aux clients Makemerank ». **Inscription libre fermée** : `POST /auth/register` → 403, Google Sign-In ne crée plus de compte inconnu ; seul chemin de création de compte restant = invitation d'équipe → prévoir une création de compte par le Super Admin. **2ᵉ passe (même jour)** : temps corrigés sur tout le site — édition de fiches / réponse aux avis / stats-mots-clés annoncées au **futur proche** (non implémentées, seul le socle OAuth + `verifyLocationMatch` est livré), consultation des avis reclassée « indépendant des API Google » (source DataForSEO), confidentialité §3 scindée lecture/écriture (écriture sur action explicite uniquement, aucune action automatique). Détail : `PROGRESS.md` (dernier bloc).
- **Widgets — en-tête carrousel repensé + taille des avatars (2026-08-01)** : `badge.avatarSize`/`carousel.avatarSize` (px, défaut 30, bornes 20–64). Carrousel : `headerPosition` `left` (nouveau défaut, style Elfsight, logo Google complet) | `top` (aligné badge compact, `headerGoogleMark`) ; `headerTextColor`/`headerMutedColor` réglables indépendamment des cartes. Builder : champs conditionnels via `when` dans `widget-schema.js`. Rétrocompatible, aucune migration. Détail : `PROGRESS.md` (dernier bloc) + `WIDGETS_DESIGN_FR.md`.
- **Widgets — badge compact (2026-07-25)** : `containerPadding` renommé « Marge extérieure » côté badge (« Marge intérieure » reste le libellé côté carrousel) ; nouveaux `badge.paddingY`/`paddingX` (padding interne, `-1` = auto selon la taille) ; `badge.googleMark` (`text`/`logo`/`both`/`none`) remplace la case `showGoogleLabel` → le G Google peut remplacer le mot « Google » (compact 208 → 173 px, 140 px sans étoile) ; séparateur passé au glyphe « | » avec marges resserrées. Réglages rendus contextuels : « Logo Google » masqué pour le badge, « Mention Google » masquée pour le carrousel. Rétrocompatible, aucune migration. Détail : `PROGRESS.md` (dernier bloc) + `WIDGETS_DESIGN_FR.md`.
...
- **Session G14 — Continuité geogrid + cercle polaire (2026-07-12)** : forme `'circle'` = **grille polaire** (centre + R anneaux à k×spacing, 8k points/anneau, total N² conservé ; `row`=anneau, `col`=index) ; continuité des courbes après changement de grille : migration 59 (`geogrid_scans.shape`), champs `*_comparable` calculés à la volée sur la **zone commune** (intersection lat/lng exacte, helper `rank-tracking/trend-compat.js`) dans `GET /trend` + `/competitors/trend`, marqueurs de rupture sur les courbes (`frontend/src/lib/geogrid-geometry.js` + prop `markers` de `TrendChart`), flèche d'évolution neutralisée (— + tooltip) entre grilles différentes, avertissement + confirmation dans le wizard Config avant de modifier une grille ayant des rapports. Détail : `PROGRESS.md` G14.
- **Terminé et vérifié** : auth/OAuth Google, onboarding, collecte publique + QR, clients (chiffrement + consentement), invitations/campagnes (Brevo OK, Twilio stub), lecture des avis via DataForSEO (GMB abandonné en lecture, module `google/` OAuth inerte), widgets (5 styles, builder, anti-fuite), crédits/plans/Stripe (webhooks), geogrid complet G1→G14 (wizard config, suivi, concurrents, cron résilient retry 3 niveaux + circuit-breaker, portail débit DataForSEO partagé), avis concurrents AC1→AC3, panel Super Admin `/admin/*` (plans/comptes/modules/**planning**/**crédits**), paramètres entreprise, gestion d'équipe (session 30), **profil & sécurité (session 31 — page `/account`)**, **facturation par propriétaire + packs/coûts crédits (session 32b)**, responsive admin, correctif transverse isolation par localisation.
...
- Carte Google Maps et événements internes Recharts ne se testent pas en preview headless — pas un bug, vérifier en vrai navigateur. Le viewport headless peut démarrer à 0 px de large → `ResponsiveContainer` rend un chart vide ; forcer `preview_resize` + recharger.
- **Recharts 3** : le label d'une `ReferenceLine` passe par un enfant `<Label value=... position="top">` (la prop objet `label={{...}}` rend le texte dans un layer séparé du groupe — vérifier dans tout le SVG, pas dans le `<g>` de la ligne).
- **Geogrid forme cercle = grille polaire depuis G14** : `row`=n° d'anneau, `col`=index dans l'anneau (plus des coordonnées cartésiennes) — tout code qui suppose row/col cartésiens pour la forme cercle est faux. Les comparaisons inter-scans se font par coordonnées lat/lng exactes (`trend-compat.js`), jamais par row/col entre formes différentes.
